...
if (isset($_POST['upload_file'])) {
    ...
    if($fileExtension != "csv") {
        ...
        } else if(strpos($sourceFile, "qua") === 0) {
            while (($row = fgetcsv($file, 0, ","))) {
                if($count > 0){
                    ...
                    $row [5] = ($row [5] == "") ? 0.0:$row [5];
                    $sql = "insert into assignment2 (time, latitude, longitude, depth, mag, gap, id, place, locationSource) VALUES ('".$row [0]."', ".$row [1]."," .$row [2]."," .$row [3]."," .$row [4].", " .$row [5].",'" .$row [6]."','" .$row [7]."','" .$row [8]."')";
                    $result = db2_exec($conn, $sql);
                }
            $count++;
            }
